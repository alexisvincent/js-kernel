...
new HotReloader.default('http://' + window.location.hostname + ':' + (window.location.port == '' ? '80' : window.location.port))
...
const build = ({
    ...
    if (kern.store == undefined) {
        ...
        kern.store = createStore(reducer, initialState, flowRight(
            ...
            ...enhancers
            // ...(production ? [] : [DevToolsEnhancer.instrument()])
            )
        )
            ...
            div({},
                $(Provider, {store: kern.store},
                    $(Router, {history: kern.history},
                        ...routes)),
                // production ? null : DevTools({ store })
            )
        )
    }
